...
- `[WIP]` Broaden SSE (started). `[DONE]` reusable `change_stream` helper (api `sse.rs`) +
  `useChangeStream` hook (web-ui) + dashboard runs feed: `GET /api/v1/runs/stream` pings on
  the `arbiter_runs` notify channel; RunsPage and JobDetail history invalidate `['runs']` on
  ping and dropped their fixed polls (RunsPage defaults to Live, poll selector kept as an
  optional override). Verified live (connect ping + a second ping on run-now).
  `[DONE]` live run view: `get_run` store method (both backends + conformance) +
  `GET /runs/{id}` + `GET /runs/{id}/stream` (payload `snapshot_stream` helper, closes on
  terminal). The worker streams subprocess stdout/stderr to the run row as it runs (chunk
...
- `[WIP]` Append-only log storage (replaces the whole-blob rewrite). `[DONE]` foundation: a
  ...
  (chunks for both streams + multiplexed SSE). `[DONE]` blob removed: dropped `stdout`/`stderr`
  columns from `job_runs`, the `JobRun` struct, and `update_run_output`. Output lives only in
  `run_log_chunks` now (no double storage, off the hot table); finalize/reschedule/get/list no
  longer touch it. Conformance + full-flow updated to read chunks. `[PLANNED]` optional capped
  tail `preview` on the runs list; virtualized scroll for very large logs (the viewer renders
  the paged-in window today). LOG STORAGE TRACK COMPLETE.
